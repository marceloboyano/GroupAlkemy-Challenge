...
public class AccountsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAccounts(int page)
    {
        ...
        if (totalPages < page) return NotFound(PAGE_NOT_FOUND_MESSAGE);
    ...
    public async Task<ActionResult> PostDeposit(int id, int amount)
    {
        ...
        var result = await _accountsService.Deposit(int.Parse(userIdFromToken), id, amount);
        if (result.Success)
            return Ok(result.Message);
    ...
    public async Task<ActionResult> PostTransfer(int id, int amount, int toAccountId)
    {
        ...
        var result = await _accountsService.Transfer(int.Parse(userIdFromToken), id, amount, toAccountId);
        if (result.Success)
            return Ok(result.Message);
        return BadRequest(result.Message);
    }
