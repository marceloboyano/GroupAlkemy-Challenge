...
public class AccountService : IAccountService
{
    ...
    public async Task<AccountForShowWithDetailsDTO?> GetAccountById(int id)
    {
        var account = await _unitOfWork.AccountRepository.GetById(id);
        if (account is null)
            return null;

        var accountDTO = _mapper.Map<AccountForShowWithDetailsDTO>(account);
        return accountDTO;
    }
    ...
    public async Task<(bool Success, string Message)> Deposit (int userId, int accountId, int amount)
    {
        ...
        if (account.User_id != userId)
            return (false, Message: ACC_NOT_MATCHED_MESSAGE);
        ...
        if (await _unitOfWork.SaveChangesAsync() > 0)
            return (Success: true, Message: ACC_DEPOSIT_SUCCESSFUL_MESSAGE);

        return (Success: false, Message: DB_NOT_EXPECTED_RESULT_MESSAGE);
    }

    public async Task<(bool Success, string Message)> Transfer(int userId, int accountId, int amount, int toAccountId)
    {
        if (amount <= 0)
            return (false, Message: ACC_AMOUNT_LESS_THAN_ZERO_MESSAGE);
    ...
    public async Task<(bool Success, string Message)> Unblock(int id)
    {
        ...
        if (await _unitOfWork.SaveChangesAsync() > 0)
            return (Success: true, Message: ACC_UNBLOCK_SUCCESSFUL_MESSAGE);
